...
import { Zap, Mail, User, Lock, ArrowRight, AlertCircle, KeyRound, CheckCircle2, ArrowLeft } from 'lucide-react';
import { useRouter } from 'next/navigation';
import { useAuth } from '@/context/auth-context';

export default function RegisterPage() {
  const router = useRouter();
  const { user, sendOtp, verifyOtp, register } = useAuth();
      ...
      <div className="w-full max-w-md glass-panel p-8 rounded-3xl border border-slate-200/80 dark:border-slate-800 space-y-6 shadow-xl relative overflow-hidden">
        <div className="text-center space-y-2">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400">
            {step === 'DETAILS' ? 'Get your secure MCP cloud workspace in seconds' : `Enter the 6-digit code sent to ${email}`}
          </p>
        </div>
        ...
        {step === 'DETAILS' ? (
          <div className="space-y-4">
            ...
            {authMode === 'PASSWORD' ? (
              <form onSubmit={handlePasswordRegister} className="space-y-4">
                ...
                >
                  <span>{loading ? 'Creating Workspace...' : 'Create Account with Password'}</span>
          ...
          <form onSubmit={handleVerifyOtp} className="space-y-4">
            ...
            <button
              ...
              className="w-full py-3 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs shadow-md flex items-center justify-center gap-2 transition-all hover:scale-[1.01] disabled:opacity-50"
            >
              <span>{loading ? 'Creating Workspace...' : 'Verify & Create Workspace'}</span>
              <ArrowRight className="h-4 w-4" />
            </button>
